...
{
  int min_model_inputs = adaptive ? batch_size : batch_size * num_samples;
...
bool ImageGenerator::load(std::vector<int>num_in_features, std::vector<int> num_out_features)
{
  ...
  for (int x = 0; x < config.inFeatures.size(); x++)
  {
    ...
    adaptive = config.rayMarchSampler[x] == "FromClassifiedDepthAdaptive" || config.rayMarchSampler[x] == "FromClassifiedDepthAdaptiveNoDepthRange";
  ...
  if (contexts.size() == 2)
  {
    if (feature_sets[0]->type == "RayMarchFromPoses")
    {
      ...
      for (int b_start = 0; b_start < settings.total_size; b_start += batch_size)
      {
        ...
        if (render_oracle)
          copyResultSamplingNetwork((float*) d_outputs[0], output_surf, batch_size, b_start, settings.width, 128);
        ...
        if (!render_oracle)
        {
          ...
          current_bi = std::chrono::steady_clock::now();
          int num_act_inputs =
            feature_set->updateFeatures(camera.getPosition(), camera.getRotMatrix(), camera.getFeaturesClean(), camera.getFeaturesModified(),
          ...
          feature_set->raymarch(d_outputs[1], output_surf, batch_size, b_start, 
                                settings.width, settings.height, camera.getDeviceFeatureVector(), d_outputs[0]);
          c += 1;
          cudaDeviceSynchronize();
          current_ai = std::chrono::steady_clock::now();

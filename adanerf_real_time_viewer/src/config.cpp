...
void Config::store(std::string key, std::string value)
{
  ...
  if (key == "adaptiveSamplingThreshold")
  {
    adaptiveSamplingThreshold = (float)atof(value.c_str());
  }
  ...
  if (key == "multiDepthFeatures")
    readIntArray(multiDepthFeatures, ",", value);

  if (key == "depthTransform")
    depthTransform = value;
  
  if (key == "accumulationMult")
    accumulationMult = value;

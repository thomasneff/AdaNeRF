...
bool NeuralRenderer::init()
{
  ...
  for (int x = 0; x < config.posEnc.size(); x++)
  {
    ...
    bool res = false;
    if(config.inFeatures[x] != "SpherePosDir" || config.raySampleInput[x] == 0)
  ...
  for (int x = 0; x < config.outFeatures.size(); x++)
  {
    ...
    if (config.outFeatures[x] == "ClassifiedDepth" || config.outFeatures[x] == "Raw" || config.outFeatures[x] == "RawSigmoid")
      num_out_features.push_back(config.multiDepthFeatures[x]);
  }

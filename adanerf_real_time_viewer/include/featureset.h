...
public:
  ...
  int getAdditionalSamples() { return additional_samples; }
...
public:
  ...
  void updateFeaturesBatched(glm::vec3 point, float* rot_mat, float* features_clean, float* features_mod,
...
private:
  ...
  float max_depth = 0;
  float adaptiveSamplingThreshold = -1.0f;
  ...
  int* d_sample_depth_idx = nullptr;
...
public:
  ...
  int updateFeatures(glm::vec3 point, float* rot_mat, float* features_clean, float* features_mod, 
                     int width, int height, int batch_offset, int batch_size, float* depth, int num_depths);

  void raymarch(void* shading_output, cudaSurfaceObject_t fb_surf, int batch_size, int batch_offset, int width, 
                int height, float* features, void* depth_values = nullptr);
};

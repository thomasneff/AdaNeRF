...
class Config(ABC):
    ...
    def init(cls, path=None, only_known_args=False):
        ...
        p.add_argument('--samplePlacementDir', type=str, help="is used when oracle is trained on precomputed sample placement")
        ...
        p.add_argument('--preTrainedSuffix', default="", type=str, help="suffix for the name of the weights (before "
                                                                        "the type extension)")
        p.add_argument('--depthTransform', default="linear", type=str,
                       help="depth transform [log, linear], default = linear",  choices=["log", "linear", "none"])
        ...
        p.add_argument('-if', '--inFeatures', default=[], action='append', type=str,
                       choices=["SpherePosDir", "CamPosDir", "RayMarchFromPoses", "RayMarchFromCoarse"])
        p.add_argument('-of', '--outFeatures', default=[], action='append', type=str,
                       choices=["ClassifiedDepth", "RGBARayMarch", "Raw", "RawSigmoid"])
        ...
        p.add_argument('--losses', default=[], type=str,
                       choices=["none", "None", "MSE", "LimitedDepthMSE",
                                ...
                                "CrossEntropyLossWeighted", "MSEPlusWeightAccum", "NeRFWeightMultiplicationLoss"],
        ...
        p.add_argument('-amp', '--amp', default=False, action="store_true", help="Activates automatic mixed precision "
                                                                                 "training")
        ...
        p.add_argument("--rayMarchSampler", default=[], type=str, action='append',
                       choices=["none", "LinearlySpacedZNearZFar", "LinearlySpacedFromDepth", "UnitSphereLinearOutsideLog", "LinearlySpacedFromDepthNoDepthRange",
                                "LinearlySpacedFromMultiDepth", "FromClassifiedDepth", "FromDepthCells",
                                "FromClassifiedDepthAdaptive", "LinearlySpacedZNearZFarNoDepthRange", "FromClassifiedDepthAdaptiveNoDepthRange"])
        p.add_argument("--adaptiveSamplingThreshold", default=-1.0, type=float,
                       help="the threshold value for adaptive sampling")
        ...
        p.add_argument('--useNerfDepthMap', default=False, action="store_true")
        p.add_argument('--useNDC', default=False, action="store_true")
        ...
        p.add_argument("--accumulationMult", default=None, type=str)
        p.add_argument("--lossComponents", default=[], action="append", type=str)
        p.add_argument("--lossComponentBlending", default=[], action="append", type=float)
        p.add_argument("--lossBlendingStart", default=-1, type=int)
        p.add_argument("--lossBlendingDuration", default=-1, type=int)

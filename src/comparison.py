...
class ExperimentResults:
    def __init__(self, path):
        ...
        match = re.search(r"LSfCDA_\((\d+\.\d+)\)", self.experiment_name)
        if match is not None:
            matches = re.findall(r'\d+\.\d+', self.experiment_name[match.start():match.end()])

            self.thresholds = matches[0]
        ...
        opt_epoch_path = os.path.join(path, "opt.txt")
        ...
        opt_epoch_ok = os.path.exists(opt_epoch_path)
        ...
        if opt_epoch_ok:
            self.read_optimal_epoch(opt_epoch_path)
...
def read_quality_info(path):
    metric_names = ["mse", "psnr", "ssim", "flip", "samples", "sparsity"]
    metrics = {"mse": [], "psnr": [], "ssim": [], "flip": [], "samples": [], "sparsity": []}
...
class ComparisonPrinter(ABC):
    file_type = None
    metric_names = ["samples", "mse", "psnr", "ssim", "flip", "sparsity"]
...
class CSVComparisonPrinter(ComparisonPrinter):
    ...
    def get_header_line(self):
        return f"Experiment_Name,Num_Parameters,Threshold,FLOPS,FLOPS_Per_Pixel,Optimal_Epoch," \
               f"Samples_Average,MSE_Average,PSNR_Average,SSIM_Average,FLIP_Average,Sparsity_Average," \
               f"Samples_Average_Video,MSE_Average_Video,PSNR_Average_Video,SSIM_Average_Video,FLIP_Average_Video," \
               f"Sparsity_average_Video\r"

    def result_to_string(self, result):
        r_string = f"{result.experiment_name},{result.params},{result.thresholds},{result.flops}," \
                   f"{result.flops_per_pixel},{result.epoch}"

...
from nerf_raymarch_common import nerf_get_ray_dirs, nerf_raw2outputs, nerf_sample_pdf, \
    nerf_get_normalization_function, nerf_get_normalization_function_abbr, adaptive_raw2outputs, ndc_rays
...
class FeatureSetKeyConstants:
    ...
    adaptive_sample_positions = "AdaptiveSamplePositions"
    oracle_weights = "OracleWeights"
    nerf_alpha_output = "NeRFAlphaOutput"
...
class Raw(FeatureSet):
    ...
    def __init__(self, config=None, net_idx=-1, device="cpu", **kwargs):
        ...
        self.abbr = f"R-{self.n_feat}"
        self.target_tensor = None

    def initialize(self, config, dataset_info, device):
        self.target_tensor = torch.tensor(True, dtype=torch.bool, device=device)
...
class RawSigmoid(Raw):
    def __init__(self, config=None, net_idx=-1, device="cpu", **kwargs):
        super(RawSigmoid, self).__init__(config, net_idx, device, **kwargs)

        self.abbr = f"S-{self.n_feat}"

    def postprocess(self, inference_dict, idx):
        inference_dict[FeatureSetKeyConstants.postprocessed_network_output] = \
            torch.sigmoid(inference_dict[FeatureSetKeyConstants.network_output])
...
class RayMarchFromPoses(FeatureSet):
    ...
    def __init__(self, config=None, net_idx=-1, device="cpu", **kwargs):
        ...
        self.device = device
        self.useNDC = config.useNDC is True
        self.adaptive = "Adaptive" in config.rayMarchSampler[net_idx]
        self.remapping = config.adaptiveSamplingThreshold > 0.0
        self.accumulation_mult = None

        if config.accumulationMult:
            self.accumulation_mult = config.accumulationMult
    ...
    def get_string(self):
        ret = self.abbr + f"[{self.z_sampler.get_name()}]"
        if self.accumulation_mult:
            ret += f"_acc_{self.accumulation_mult}"
        return ret

    def initialize(self, config, dataset_info, device):
        self.view_cell_center = torch.tensor(dataset_info.view.view_cell_center, device=self.device)
        ...
        self.depth_range = dataset_info.depth_range if not dataset_info.use_warped_depth_range[self.net_idx] else dataset_info.depth_range_warped
    ...
    def prepare_batch(self, data, config):
        ...
        if DatasetKeyConstants.sample_placement in data:
            ret_dict[DatasetKeyConstants.sample_placement] = data[DatasetKeyConstants.sample_placement]
    ...
    def batch(self, data, **kwargs) -> (torch.tensor, torch.tensor):
        ...
        if DatasetKeyConstants.sample_placement in data:
            sample_placement = data[DatasetKeyConstants.sample_placement]
        ...
        if prev_outs is not None and len(prev_outs) > 0 and (not self.train_with_gt_depth or is_inference is True):
            depth = prev_outs[-1][FeatureSetKeyConstants.postprocessed_network_output]
        else:
        ...
        # TODO if NDC and oracle, use basic inputs in sphereposdir
        ...
        if self.useNDC:
            ray_origins, rays_d = ndc_rays(self.h, self.w, self.view.focal, 1., ray_origins, ray_directions)
            ray_directions = rays_d / torch.norm(rays_d, dim=-1, keepdim=True)
        ...
        z_vals = self.z_sampler.generate(n_images * n_samples, poses.device, depth=depth, depth_range=self.depth_range,
                                 ...
                                 num_ray_samples=self.n_ray_samples, ray_origins=ray_origins, ray_directions=ray_directions)
        mapping = None
        if self.adaptive and self.remapping:
            if isinstance(z_vals, tuple):
                temp = z_vals
                z_vals = temp[0]
                z_probs = temp[1]

            mapping = float("inf") > z_vals
            mapping = torch.flatten(mapping)
        ...
        ray_sample_positions = (ray_origins[..., None, :] + rays_d[..., None, :] * z_vals[..., :, None])
        ...
        if self.adaptive and self.remapping:
            embedded = embedded[mapping, :]
            z_vals = torch.flatten(z_vals)
            z_vals = torch.masked_select(z_vals, mapping)
        #else:
        ...
        ret_dict = {FeatureSetKeyConstants.input_feature_batch: embedded,
                    ...
                    FeatureSetKeyConstants.nerf_input_feature_ray_directions: rays_d,
                    FeatureSetKeyConstants.nerf_input_feature_ray_origins: ray_origins}

        if self.adaptive and self.remapping:
            ret_dict[FeatureSetKeyConstants.adaptive_sample_positions] = mapping
        ...
        if self.config.losses[0] == "NeRFWeightMultiplicationLoss":
            ret_dict[FeatureSetKeyConstants.oracle_weights] = depth if z_probs is None else z_probs
    ...
    def postprocess(self, inference_dict, idx):
        ...
        batch_input = inference_dict[FeatureSetKeyConstants.input_feature_batch]

        depth = None
        if FeatureSetKeyConstants.oracle_weights in inference_dict:
            depth = inference_dict[FeatureSetKeyConstants.oracle_weights]

        if self.adaptive:
            n_images = idx[DatasetKeyConstants.image_pose].shape[0]
            n_samples = idx[DatasetKeyConstants.ray_directions_samples].shape[1]

            mapping = None
            if FeatureSetKeyConstants.adaptive_sample_positions in inference_dict:
                mapping = inference_dict[FeatureSetKeyConstants.adaptive_sample_positions]

            rgb_map, disp_map, acc_map, weights, depth_map, alpha = adaptive_raw2outputs(network_out, z_vals, ray_directions,
                                                                                  n_images=n_images,
                                                                                  n_samples=n_samples,
                                                                                  mapping=mapping,
                                                                                  depth=depth,
                                                                                  accumulation_mult=self.accumulation_mult)

            if self.remapping:
                max_num_ray_samples = int(mapping.shape[0] // (n_images * n_samples))

                restored_z = torch.zeros((mapping.shape[0], 1), dtype=torch.float32, device=mapping.device)
                restored_input = torch.zeros((mapping.shape[0], batch_input.shape[-1]), dtype=torch.float32,
                                             device=mapping.device)
                restored_out = torch.zeros((mapping.shape[0], 4), dtype=torch.float32, device=mapping.device)

                mapping_index = torch.nonzero(mapping)

                restored_z[mapping_index, 0] = torch.unsqueeze(z_vals, 1)[:, None, 0]
                restored_z[restored_z == 0] = float("NaN")
                restored_z = restored_z.view(n_images * n_samples, max_num_ray_samples)

                restored_input[mapping_index, :] = batch_input[:, None, :]
                restored_input[restored_input == 0] = float("NaN")
                restored_input = restored_input.view(n_images * n_samples, max_num_ray_samples, -1)

                restored_out[mapping_index, :] = network_out.to(torch.float32)[:, None, :]
                restored_out = restored_out.view(n_images * n_samples, max_num_ray_samples, -1)

                inference_dict[FeatureSetKeyConstants.nerf_input_feature_z_vals] = restored_z
                inference_dict[FeatureSetKeyConstants.input_feature_batch] = restored_input
                inference_dict[FeatureSetKeyConstants.network_output] = restored_out

                mapping = mapping.view(n_images * n_samples, int(mapping.shape[0] // (n_images * n_samples)))
                mapping = torch.sum(mapping, 1) / self.n_ray_samples
                inference_dict[FeatureSetKeyConstants.adaptive_sample_positions] = mapping
        else:
            rgb_map, disp_map, acc_map, weights, depth_map, alpha = nerf_raw2outputs(
                network_out.reshape(ray_directions.shape[0], z_vals.shape[1], -1), z_vals, ray_directions, depth=depth,
                accumulation_mult=self.accumulation_mult)
        ...
        inference_dict[FeatureSetKeyConstants.nerf_alpha_output] = alpha

        if self.useNDC:
            inference_dict[FeatureSetKeyConstants.nerf_estimated_depth] = torch.reshape(depth_map, (-1, 1))
        else:
...
class RayMarchFromCoarse(FeatureSet):
    ...
    def initialize(self, config, dataset_info, device):
        ...
        self.depth_transform = dataset_info.depth_transform
        self.max_depth = dataset_info.depth_max
        self.view_cell_center = torch.tensor(dataset_info.view.view_cell_center, dtype=torch.float32, device=device)
...
class SpherePosDir(FeatureSet):
    ...
    def initialize(self, config, dataset_info, device="cpu"):
        ...
        self.view_cell_radius = torch.tensor(np.linalg.norm((np.array(dataset_info.view.view_cell_size) / 2.0)))
        ...
        self.view_cell_center_gpu = self.view_cell_center.clone().detach().to(self.device)
        self.view_cell_radius_gpu = self.view_cell_radius.clone().detach().to(self.device)
    ...
    def batch(self, data, **kwargs) -> (torch.tensor, torch.tensor):
        ...
        encnds = self.dir_enc.encode(nds / torch.norm(nds, dim=-1, keepdim=True)).reshape(n_images, n_samples, -1)
...
class CamPosDir(FeatureSet):
    def __init__(self, config=None, net_idx=-1, device="cpu", **kwargs):
        ...
        if config.raySampleInput:
            self.additionalSamples = config.raySampleInput[net_idx]
        if self.additionalSamples != 0:
            self.abbr = f"CaPoDi[{self.additionalSamples}]"

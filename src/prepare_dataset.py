...
def main():
    ...
    parser.add_argument("-d", "--device", type=int, default=0, help="the device id to be used (-1 means cpu)")
    ...
    for s in splits:
        ...
        for frame_idx in tqdm_range:
            ...
            nerf_depth_name = os.path.join(path, frame['file_path'] + "_QuantizedWeights_lo_nSD.raw")

            if os.path.exists(depth_file):
                depth_image = load_depth_image(depth_file, h, w, flip_depth)
            elif os.path.exists(nerf_depth_name):
                quantized_weights_dict_full = torch.load(nerf_depth_name)
                ...
                print("Error: No depth data found!")
                sys.exit(-1)
    ...
    if exported_depth_range is None:
        for s in splits:
            with open(os.path.join(path, f"transforms_{s}.json"), "r") as f:
                transforms_data = json.load(f)
        ...
        depth_range = [exported_depth_range[0], exported_depth_range[1]]
        max_depth = 1.01 * depth_range[1]
        min_z = depth_range[0]
        max_z = depth_range[1]
    ...
    view_cell_radius = torch.tensor(view_cell_size[0] ** 2 + view_cell_size[1] ** 2 + view_cell_size[2] ** 2,
                                    dtype=torch.float32, device="cpu")
    view_cell_radius = 0.5 * torch.sqrt(view_cell_radius)
    ...
    for s in splits:
        ...
        for frame_idx in tqdm_range:
            ...
            if os.path.exists(depth_file):
                ...
                quantized_weights_dict_full = torch.load(nerf_depth_name)
                depth_image = depth_transforms.LinearTransform.to_world(quantized_weights_dict_full['OutputDepthMap'].cpu().numpy(), depth_range)

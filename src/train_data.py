...
class TrainConfig:
    ...
    def initialize(self, config, load_data=True, log_path=None, training=True):
        ...
        self.initialize_features(config.storeFullData)
        ...
        experiment_name = ndc_str + nerf_depth + depth_transform + scale_interpolation + \
                          config_to_name(self.f_in, self.f_out, self.models, self.encodings, self.enc_args,
                                         config.losses, config.lossWeights, config.lossComponents,
                                         config.lossComponentBlending, config.lossBlendingStart,
                                         config.lossBlendingDuration, config.lossAlpha, config.lossBeta)
    ...
    def initialize_features(self, on_device):
        for f in self.f_in:
            f.initialize(self.config_file, self.dataset_info, self.device if on_device else "cpu")
        for f in self.f_out:
            f.initialize(self.config_file, self.dataset_info, self.device if on_device else "cpu")
    ...
    def save_weights(self, name_suffix, model_idx=-1):
        ...
        if self.config_file.amp:
            torch.save(self.scaler.state_dict(), os.path.join(self.logDir, f"{name_suffix}.scale"))

    def save_weights_in_path(self, path, name_suffix, model_idx=-1):
        for i, model in enumerate(self.models):
            if model_idx == -1 or model_idx == i:
                #model.delete_saved_weights(self.logDir, optimizer=self.optimizers[i])
                model.save_weights(path, name_suffix=name_suffix, optimizer=self.optimizers[i])

        if self.config_file.amp:
            torch.save(self.scaler.state_dict(), os.path.join(path, f"{name_suffix}.scale"))
    ...
    def load_latest_weights(self):
        ...
        for i, model in enumerate(self.models):
            ...
            if self.epoch0 == 1 and self.config_file.preTrainedSuffix != "" and self.config_file.adaptiveSamplingThreshold > 0:
                base_experiment_name = re.sub(r"\d+_LSfCDA_\(\d+\.\d+\)", "128_LSfCDA_(0.0)", self.experiment_name)

                scaler_path = os.path.join(self.config_file.preTrained[i], base_experiment_name)
                scaler_name = self.config_file.preTrainedSuffix

                model.load_specific_weights(os.path.join(self.config_file.preTrained[i], base_experiment_name),
                                            self.config_file.preTrainedSuffix, self.optimizers[i], self.device)

        if scaler_path is None:
            scaler_path = self.logDir
            scaler_name = f"{self.epoch0 - 1:07d}"

        if self.config_file.amp:
            self.load_scaler_state_dict(scaler_path, scaler_name)
    ...
    def load_specific_weights(self, name, model_idx=-1):
        ...
        if self.config_file.amp:
            self.load_scaler_state_dict(self.logDir, name)

    def load_scaler_state_dict(self, path, name):
        if os.path.exists(os.path.join(path, f"{name}.scale")):
            print('Reloading scaler Checkpoint from', os.path.join(path, f"{name}.scale"))
            scaler_state = torch.load(os.path.join(path, f"{name}.scale"))

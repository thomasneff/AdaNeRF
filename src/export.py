...
def export_onnx(train_config=None, out_dir=None):
    ...
    with open(f"{out_dir}/dataset_info.txt", "w") as f:
        ...
        f.write("depth_range = " + str(train_config.dataset_info.depth_range_warped) + "\n")
    ...
    for b1 in img_samples.batches(128):
        ...
        for i in range(len(inference_dicts)):
            if i == len(inference_dicts) - 1 and len(inference_dicts[i]['InputFeatureBatch'].shape) == 3:
                in_tensor = inference_dicts[i]['InputFeatureBatch']
                input_feature_batches.append(in_tensor.reshape(in_tensor.shape[0] * in_tensor.shape[1], -1))
            else:

        ...
        if len(ckpts) == 0:
            ...
            if config and config.preTrained and len(config.preTrained) > self.net_idx and config.preTrained[self.net_idx].lower() != "none":
                ...
                if os.path.exists(wpath):
                    ...
                    wpath = os.path.join(config.preTrained[self.net_idx], f"{self.name}__opt.weights")
                    if os.path.exists(wpath):
                        print("loading pretrained weights")
                        self.load_weights(wpath, device)
                    else:
                        print(f"WARNING pretrained weights not found: {wpath}")
                opath = wpath = os.path.join(config.preTrained[self.net_idx], f"{self.name}{config.preTrainedSuffix}.optimizer")
                if optimizer is not None and os.path.exists(opath):
                    self.load_optimizer(opath, optimizer, device)
                else:
                    opath = wpath = os.path.join(config.preTrained[self.net_idx],
                                                 f"{self.name}{config.preTrainedSuffix}__opt.optimizer")
                    if optimizer is not None and os.path.exists(opath):
                        self.load_optimizer(opath, optimizer, device)
            return 0
        ckpt_path = ckpts[-1]

...
from util.helper import t2np, init_full_image_dict, add_inference_dict
...
def generate_data(train_config, flags, reference_video=None):
    ...
    for i in tqdm(range(len(dataset)), desc=desc, position=0, leave=True):
        ...
        for batch in img_data.batches(train_config.config_file.inferenceChunkSize):
            img_part, inf_dict = train_config.inference(batch, gradient=False, is_inference=True)

            if FeatureSetKeyConstants.adaptive_sample_positions in inf_dict[-1]:
                samples += torch.sum(inf_dict[-1][FeatureSetKeyConstants.adaptive_sample_positions])
            ...
            if inference_dict_full_list is None:
                inference_dict_full_list = init_full_image_dict(train_config, inf_dict, complete=False)

            add_inference_dict(inf_dict, inference_dict_full_list, start_index, end_index)

            start_index = end_index

        if samples > 0.:
            samples = (samples / (dim_h * dim_w)) * train_config.config_file.numRaymarchSamples[-1]
            q_cont.samples.append(samples)
        else:
            samples = train_config.config_file.numRaymarchSamples[-1]

        if count_flops:
            total_macs = 0
            for k in range(len(train_config.models)):
                macs, params = train_config.models[k].compute_average_flops_cost()
                total_macs += macs if k == 0 else macs * samples
        ...
        if train_config.config_file.adaptiveSamplingThreshold == 0.0:
            sparsity = torch.mean(inference_dict_full_list[1][features.FeatureSetKeyConstants.oracle_weights])

            q_cont.sparsity.append(sparsity)
    ...
    with open(f"{train_config.outDir}/image_quality_{suffix}.txt", "w") as f:
        for idx, mse in enumerate(q_cont.mse):
            f.write(f"image={idx} mse={mse:.4f} psnr="
                    ...
                    f"flip_loss="
                    f"{q_cont.flip[idx] if 'flip' in flags else -1.:.4f} "
                    f"samples="
                    f"{q_cont.samples[idx] if len(q_cont.samples) > 0 else float(train_config.config_file.numRaymarchSamples[-1])} "
                    f"sparsity="
                    f"{q_cont.sparsity[idx] if len(q_cont.sparsity) > 0 else -1.:.4f}\r")

    with open(f"{train_config.outDir}/image_quality_{suffix}.csv", "w") as c:
        c.write(f"mse,psnr,ssim,flip,samples,sparsity\r")
        for idx, mse in enumerate(q_cont.mse):
            c.write(f"{mse},{q_cont.psnr[idx] if 'psnr' in flags else -1.},"
                    f"{q_cont.ssim[idx] if 'ssim' in flags else -1.},"
                    f"{q_cont.flip[idx] if 'flip' in flags else -1.},"
                    f"{q_cont.samples[idx] if len(q_cont.samples) > 0 else float(train_config.config_file.numRaymarchSamples[-1])},"
                    f"{q_cont.sparsity[idx] if len(q_cont.sparsity) > 0 else -1.}\r")
...
def evaluate(train_config, reference_video, evaluations):
    train_config.config_file.outputNetworkRaw = [features.FeatureSetKeyConstants.oracle_weights]
...
def load_config(data_path, device_id, path, evaluations, skip, cl_out_dir=None, skip_if_already_done_once=True, load_training_datasets=False):
    ...
    train_config.initialize(config, log_path=orig_path, training=load_training_datasets)
